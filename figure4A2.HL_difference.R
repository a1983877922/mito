...
for (i in 1:nrow(ped)) {
  ...
  inherit_df <- full_join(child_df,mother_df,by="id") %>% 
    ...
    mutate(HID = child,
           MHID = mother) %>% 
    select(id,pos,ref,alt,info,diff,HID,MHID)
  mergedf<- rbind(mergedf,inherit_df)

}

mergedf <- mergedf %>% 
  mutate(
    logdiff = sign(diff) * log1p(abs(diff))
  )
  
  # mutate(
  #   logdiff = case_when(
  #     diff > 0 ~ log1p(diff),
  #     diff < 0 ~ -log1p(-diff),  # 对负值取负对数
  #     diff == 0 ~ 0
  #   )
  # )
  # mutate(
  #   logdiff = log(abs(diff) +1 )
  # )
